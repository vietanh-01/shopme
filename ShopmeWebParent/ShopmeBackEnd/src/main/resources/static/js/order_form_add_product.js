...
function addProduct(productId, productName) {
	if (isProductAlreadyAdded(productId)) {
		showWarningModal("Product has already been added.");
		return;
	}
...
function getProductInfo(productId, shippingCost) {
	...
	$.get(requestURL, function(productJson) {
		...
		productCost = parseFloat(productJson.cost).toFixed(2);
		productPrice = parseFloat(productJson.price).toFixed(2);
...
function generateProductCode(productId, productName, mainImagePath, productCost, productPrice, shippingCost) {
	...
	productCostFormatted = $.number(productCost, 2, '.', ',');
	productPriceFormatted = $.number(productPrice, 2, '.', ',');
				...
				<tr>
					...
					<td>
						<input type="text" required class="form-control m-1 cost-input"
							...
							rowNumber="${nextCount}" 
							value="${productCostFormatted}" style="max-width: 140px"/>
					</td>
				</tr>
				<tr>
					<td>Quantity:</td>
				...
				<tr>
					...
					<td>
						...
						name="productPrice"
							id="${priceId}"
							rowNumber="${nextCount}" 
							value="${productPriceFormatted}" style="max-width: 140px"/>
					</td>
				</tr>
				<tr>
					<td>Subtotal:</td>
				...
				<tr>
					...
					<td>
						...
						name="productShipCost" 
							value="${shippingCostFormatted}" style="max-width: 140px"/>
					</td>
				</tr>											
			</table>
			</div>
