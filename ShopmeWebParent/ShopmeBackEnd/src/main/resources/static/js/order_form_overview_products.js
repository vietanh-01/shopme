...
function setAndFormatNumberForField(fieldId, fieldValue) {
	formattedValue = $.number(fieldValue, 2, '.', ','); // Specify the decimal and thousand separators
	$("#" + fieldId).val(formattedValue);
}

function getNumberValueRemovedThousandSeparator(fieldRef) {
	fieldValue = fieldRef.val().replace(/,/g, ''); // Remove commas
	return parseFloat(fieldValue); // Parse the cleaned value as a float
}
...
function formatNumberForField(fieldRef) {
	fieldRef.val($.number(fieldRef.val(), 2, '.', ',')); // Specify the decimal and thousand separators
}
